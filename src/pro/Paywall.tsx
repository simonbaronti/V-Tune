...
import { signInWithEmail, signInWithProvider } from './auth';
    ...
    <div className="flex flex-col gap-2.5">
      {sent ? (
        ...
        <>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => void signInWithProvider('apple').then((r) => r.error && setError(r.error))}
            ...
            <button
              onClick={() => void signInWithProvider('google').then((r) => r.error && setError(r.error))}
              className="flex-1 rounded px-3 py-2 text-sm font-semibold"
              style={{ background: 'var(--bg-tertiary)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}
            >
              Sign in with Google
